...
using VelthovenConsultancy.AgileTool.Migrations.SeedData;
...
namespace VelthovenConsultancy.AgileTool.Tests
{
    public abstract class AgileToolTestBase : AbpIntegratedTestBase<AgileToolTestModule>
    {
        ...
        protected void UsingDbContext(Action<AgileToolDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AgileToolDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AgileToolDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AgileToolDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AgileToolDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AgileToolDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AgileToolDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AgileToolDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AgileToolDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AgileToolDbContext, Task<T>> func)
        {
            T result;
